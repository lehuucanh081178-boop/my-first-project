...
function filterCategory(cat, btn) {
  ...
  loadReaders(cat); // gọi API thay vì filter local
}
...
async function submitBooking(e) {
  ...
  if (!Auth.isLoggedIn()) {
    ...
    showToast('⚠️ Vui lòng đăng nhập để đặt lịch');
  ...
  try {
    ...
    showToast('❌ ' + (err.message || 'Lỗi đặt lịch, thử lại'));
  } finally {
    btn.textContent = 'Xác Nhận Đặt Lịch';
    btn.disabled = false;
  }
...
async function submitLogin(e) {
  ...
  try {
    ...
    showToast('❌ ' + (err.message || 'Email hoặc mật khẩu không đúng'));
  } finally {
...
async function submitRegister(e) {
  ...
  try {
    ...
    showToast(`🎉 Chào mừng ${result.user.name} đến với TarotLove!`);
    updateAuthUI(result.user);
  } catch (err) {
    showToast('❌ ' + (err.message || 'Lỗi đăng ký, thử lại'));
  } finally {
...
function showPaymentModal(booking) {
  ...
  document.getElementById('paymentContent').innerHTML = `
    ...
    <p style="color:var(--text-muted);font-size:.8rem;text-align:center;margin-top:12px">
      Sau khi thanh toán, chúng tôi sẽ xác nhận và liên hệ bạn trong 30 phút
    </p>
  `;
...
async function payWithMomo(bookingId) {
  try {
    showToast('⏳ Đang tạo link thanh toán MoMo...');
    const result = await PaymentAPI.momo(bookingId);
    window.open(result.payUrl, '_blank');
  } catch (err) {
    showToast('❌ ' + (err.message || 'Lỗi tạo thanh toán MoMo'));
  }
}

async function payWithBank(bookingId) {
  try {
    const result = await PaymentAPI.bank(bookingId);
    const { bankInfo } = result;
    document.getElementById('paymentContent').innerHTML = `
    ...
    showToast('❌ Lỗi lấy thông tin ngân hàng');
  }
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  initTypewriter();
  updateAuthUI();
  loadReaders(); // load từ API thật

  // Lắng nghe sự kiện login/logout
  window.addEventListener('auth:login', (e) => updateAuthUI(e.detail));
  window.addEventListener('auth:logout', () => updateAuthUI(null));
});

// ===== LOAD READERS TỪ API =====
async function loadReaders(category = 'all') {
  const grid = document.getElementById('readersGrid');
  grid.innerHTML = `
...
function updateAuthUI(user) {
  ...
  if (currentUser) {
    headerActions.innerHTML = `
      <div class="user-menu">
        ...
        <div class="user-dropdown" id="userDropdown">
          <a href="dashboard.html"><i class="fas fa-user"></i> Tài khoản</a>
          <a href="dashboard.html#bookings"><i class="fas fa-calendar"></i> Lịch đặt</a>
    ...
    headerActions.innerHTML = `
      ...
      <button class="btn-register" onclick="openModal('registerModal')">Đăng Ký</button>
    `;
  }
}
